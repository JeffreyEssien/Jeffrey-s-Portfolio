...
export async function fetchRemotive(): Promise<AdapterResult> {
  try {
    const res = await fetch('https://remotive.com/api/remote-jobs', { headers: { Accept: 'application/json' } })
    if (!res.ok) throw new Error(`remotive ${res.status}`)
    const json = await res.json() as { jobs?: RemotiveJob[] }
    const jobs: RawJob[] = (json.jobs ?? [])
      .filter((j) => /software|dev|engineer/i.test(j.category || ''))
      .map((j) => ({
